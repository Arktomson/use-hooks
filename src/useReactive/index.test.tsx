...
const Demo = () => {
  ...
  return (
    <div>
      <p>
        counter state.count：
        <span role="addCount">{state.count}</span>
      </p>
      <p>
        delete property：
        <span role="deleteProperty">{state.foo}</span>
      </p>
      ...
      <button
        role="deletePropertyBtn"
        onClick={() => delete state.foo}
      >
        delete state.foo
      </button>
      <button
...
describe('test useReactive feature', () => {
  ...
  it('test array', () => {
    ...
    expect(
      JSON.parse(testArray.textContent as any).length,
    ).toBe(2);
    act(() => {
  ...
  it('test special objects', () => {
    ...
    expect(() =>
      result.current.b.valueOf(),
    ).not.toThrowError();
    expect(() =>
      result.current.c.valueOf(),
    ).not.toThrowError();
    expect(
      () => result.current.d.message,
    ).not.toThrowError();
    expect(() =>
      result.current.e.valueOf(),
    ).not.toThrowError();
    expect(() =>
      result.current.f.valueOf(),
    ).not.toThrowError();
    expect(() => result.current.g.PI).not.toThrowError();
    expect(() =>
      result.current.h.getFullYear(),
    ).not.toThrowError();
    expect(() =>
      result.current.i.valueOf(),
    ).not.toThrowError();
    expect(() =>
      result.current.j1.test('a'),
    ).not.toThrowError();
    expect(() =>
      result.current.j2.test('a'),
    ).not.toThrowError();
    expect(
      () => result.current.k.length,
    ).not.toThrowError();
    expect(() => result.current.l.size).not.toThrowError();
    expect(() => result.current.m.size).not.toThrowError();
    expect(
      () => result.current.n.byteLength,
    ).not.toThrowError();
    expect(
      () => result.current.o.byteLength,
    ).not.toThrowError();
    expect(() =>
      result.current.p.isLockFree(1),
    ).not.toThrowError();
    expect(() =>
      result.current.q.stringify(1),
    ).not.toThrowError();
    expect(() =>
      result.current.r.then(),
    ).not.toThrowError();
    expect(() =>
      result.current.s.ownKeys({}),
    ).not.toThrowError();
    expect(() =>
      result.current.t.toString(),
    ).not.toThrowError();
    expect(() =>
      result.current.u.DateTimeFormat(),
    ).not.toThrowError();
    expect(
      () => result.current.v.Module,
    ).not.toThrowError();
  });
  ...
  it('delete object property', () => {
    ...
    const deletePropertyBtn = wrap.getByRole(
      'deletePropertyBtn',
    );
